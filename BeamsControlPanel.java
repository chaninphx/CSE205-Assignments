...
public class BeamsControlPanel extends JPanel
{
    ...
    public BeamsControlPanel(int width, int height, Color initialColor)
    {
        ...
        beamsPanel = new BeamsPanel(color, width);
    ...
    private class ButtonListener implements ActionListener
    {
        public void actionPerformed(ActionEvent event)
        {
            ...
            //if start is pressed the beam animation resumes
            //if stop is pressed the beam animation will pause
    ...
    private class SpeedListener implements ChangeListener
    {
        ...
        {
            //when a user moves the delay JSlider, the value is retrieved from the source,
            //the delay in the beam panel class is set to delayVal
            JSlider source = (JSlider)event.getSource();
            int delayVal = (int)source.getValue();
            beamsPanel.setDelay(delayVal);
    ...
    private class BeamListener implements ChangeListener
    {
        public void stateChanged(ChangeEvent event)
        {
            //same as SpeedListener but this has to do with the beam number JSlider
            //the beam number is set to beamNum
            JSlider source = (JSlider)event.getSource();
            int beamNum = (int)source.getValue();
            beamsPanel.setBeamNumber(beamNum);
        }
    } //end of BeamListener
}